...
class TestRmaCase(TestRma):
    def test_link_to_origin(self):
        delivery = self._create_delivery()
        move_line = delivery.move_lines[0]

        def _create_rma():
            rma_form = Form(self.env["rma"])
            rma_form.partner_id = delivery.partner_id
            rma_form.picking_id = delivery
            rma_form.move_id = move_line
            rma_form.product_uom_qty = 1
            rma = rma_form.save()
            rma.action_confirm()
            return rma
        ...
        self.assertEqual(rma.reception_move_ids.move_orig_ids, move_line)
        rma = _create_rma()
        self.assertEqual(rma.reception_move_ids.move_orig_ids, move_line)
        rma = _create_rma()
        self.assertEqual(rma.reception_move_ids.move_orig_ids, move_line)
