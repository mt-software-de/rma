...
class Rma(models.Model):
    ...
    def _prepare_procurement_group_values(self, name=None):
        return {
            "move_type": "direct",
    ...
    def _create_procurement_group(self, values):
        return self.env["procurement.group"].create(values)

    def _prepare_procurement_values(self, group, warehouse=None, scheduled_date=None):
        self.ensure_one()
        return {
    ...
    def _prepare_reception_procurement_values(self, group):
        values = self._prepare_procurement_values(group)
        values.update(
            {
                "route_ids": self.warehouse_id.rma_in_route_id,
            ...
            values.update(
                {
                    ...
                    "move_orig_ids": [(4, self.move_id.id)],
                }
            )
        return values

    def _prepare_reception_procurement_group_values(self):
        return self._prepare_procurement_group_values()

    def _create_reception_procurement_group(self):
        values = self._prepare_reception_procurement_group_values()
        return self._create_procurement_group(values)
    ...
    def _prepare_reception_procurement(self):
        ...
        return self._prepare_procurement(group, self.warehouse_id.rma_loc_id, values)
    ...
    def _prepare_outgoing_procurement_values(
        self, group, warehouse=None, scheduled_date=None
    ):
        values = self._prepare_procurement_values(group, warehouse, scheduled_date)
        warehouse = warehouse or self.warehouse_id
        values.update({"route_ids": warehouse.rma_out_route_id, "rma_id": self.id})
        return values

    def _prepare_delivery_procurement_values(self, group, scheduled_date=None):
        values = self._prepare_outgoing_procurement_values(group)
        values.update(
            {
                "move_orig_ids": [(4, self.reception_move_ids.id)],
            }
        )
        return values

    def _prepare_delivery_procurement(
        self, group, scheduled_date=None, qty=None, uom=None
    ):
        self.ensure_one()
        values = self._prepare_delivery_procurement_values(group, scheduled_date)
    ...
    def _prepare_delivery_procurement_group_values(self):
        ...
        return self._prepare_procurement_group_values(origin)

    def _create_delivery_procurement_group(self):
        values = self._prepare_delivery_procurement_group_values()
        return self._create_procurement_group(values)

    def _prepare_delivery_procurements(self, scheduled_date=None, qty=None, uom=None):
        procurements = []
        for rmas in self._group_if_needed():
            group = rmas._create_delivery_procurement_group()
            for rma in rmas:
                procurements.append(
                    rma._prepare_delivery_procurement(group, scheduled_date, qty, uom)
                )
        return procurements
    ...
    def _create_replace_procurement_group(self):
        values = self._prepare_replace_procurement_group_values()
        return self._create_procurement_group(values)

    def _prepare_replace_procurement_group_values(self):
        return self._prepare_procurement_group_values()

    def _prepare_replace_procurement_values(
        self, group, warehouse=None, scheduled_date=None
    ):
        return self._prepare_outgoing_procurement_values(
            group, warehouse, scheduled_date
        )
    ...
    def _prepare_replace_procurement(
        ...
        values = self._prepare_replace_procurement_values(
            self.procurement_group_id, warehouse, scheduled_date
        )
        return self._prepare_procurement(
